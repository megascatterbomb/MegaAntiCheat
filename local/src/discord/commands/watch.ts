...
@Alias("watch", "markwatch")
@Inhibit({ limitBy: "USER", maxUsesPerPeriod: 3, periodDuration: 10 })
@Described("Add a user to the database but do not mark them as anything.")
export default class WatchCommand extends ChannelCommand {
    @Argument({ type: new StringType(), description: "The steam user to watch (Link to Steam Profile or SteamID)."})
        steamUser!: string;
    ...
    async execute(message: Message, client: Client) {
        ...
        if(dbUser?.userType === SteamUserType.KnownCheater || dbUser?.userType === SteamUserType.Suspicious || dbUser?.userType === SteamUserType.Innocent) {
            await this.overrideMessage(steamID64, message, dbUser.userType);
            return;
        } else if (dbUser?.userType === SteamUserType.Watching) {
            message.reply("This user is already being watched");
            finishInteraction(steamID64);
            return;
        }
        const err = await registerSteamUser(steamID64, SteamUserType.Watching, message.author.id);
        if(err !== "") {
            throw new Error(err);
        }
        finishInteraction(steamID64);
        message.reply("Now watching `" + steamID64 + "`.");
    }
    ...
    async overrideMessage(steamID64: string, cmdMessage: Message, currentType: SteamUserType) {
        ...
        const typeString = currentType === SteamUserType.Suspicious ? "suspicious" : currentType === SteamUserType.Innocent ? "innocent" : "a cheater";
        const response = cmdMessage.reply({components: [buttonRow], content:
            `**This user has already been marked as ${typeString}.** If you wish to unmark this user while still keeping an eye on them, you can, otherwise you may cancel the operation.`});
        const interactionFunction = async (interaction: Interaction<CacheType>) => {
            ...
            try {
                if(interaction.isButton() && interaction.customId === "submitButton" + cmdMessage.id) {
                    ...
                    await interaction.editReply({ content: "Now watching `" + steamID64 + "`.", components: [] });
                    const err = await registerSteamUser(steamID64, SteamUserType.Watching, interaction.user.id);
