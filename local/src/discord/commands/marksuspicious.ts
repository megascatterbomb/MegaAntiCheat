...
import { client } from "..";
import {  getSteamUser, registerSteamUser, SteamIDType, SteamUserType } from "../database/steamUser";
...
@Alias("marksuspicious", "suspicious", "marksus", "sus")
@Inhibit({ limitBy: "USER", maxUsesPerPeriod: 3, periodDuration: 10 })
@Described("Mark a steam user as a potential cheater.")
export default class MarkSuspiciousCommand extends ChannelCommand {
    @Argument({ type: new StringType(), description: "The steam user to mark as suspicious (Link to Steam Profile or SteamID)."})
        steamUser!: string;
    ...
    async execute(message: Message, client: Client) {
        ...
        if(dbUser?.userType === SteamUserType.KnownCheater || dbUser?.userType === SteamUserType.Innocent) {
            await this.overrideMessage(steamID64, message, dbUser.userType);
            return;
        } else if (dbUser?.userType === SteamUserType.Suspicious) {
            message.reply("This user has already been marked as Suspicious");
            finishInteraction(steamID64);
            return;
        }
        const err = await registerSteamUser(steamID64, SteamUserType.Suspicious, message.author.id);
        if(err !== "") {
            throw new Error(err);
        }
        finishInteraction(steamID64);
        message.reply("Successfully marked `" + steamID64 + "` as a suspicious user.");
    }
    ...
    async overrideMessage(steamID64: string, cmdMessage: Message, currentType: SteamUserType) {
        ...
        const typeString = currentType === SteamUserType.Innocent ? "innocent" : "a cheater";
        const response = cmdMessage.reply({components: [buttonRow], content:
            `**This user has already been marked as ${typeString}.** If you wish to change this to suspicious, you can, otherwise you may cancel the operation.`});
        const interactionFunction = async (interaction: Interaction<CacheType>) => {
            ...
            try {
                if(interaction.isButton() && interaction.customId === "submitButton" + cmdMessage.id) {
                    ...
                    await interaction.editReply({ content: "Successfully marked `" + steamID64 + "` as a suspicious user", components: [] });
                    const err = await registerSteamUser(steamID64, SteamUserType.Suspicious, interaction.user.id);
