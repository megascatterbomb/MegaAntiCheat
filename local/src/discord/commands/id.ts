import {
    Alias,
    Inhibit,
    Permit,
    Command,
    Argument,
    BooleanType,
    IntegerType,
    UnionType,
    FloatType,
    Client,
    UserType,
    Described,
    StringType,
} from "@frasermcc/overcord";
import { Message, User } from "discord.js";
import { registerSteamUser, SteamIDType, SteamUserType } from "../database/steamUser";
import { registerSubscriber, toggleSubscriptionStatus } from "../database/subscriber";
import ChannelCommand from "../extensions/channelCommand";
import { convertSteamID64toSteamID1, convertSteamID64toSteamID3, getSteamID64, steam } from "../webrequests/steamquery";

@Alias("id", "steamid")
@Inhibit({ limitBy: "USER", maxUsesPerPeriod: 3, periodDuration: 10 })
@Described("Link your steam account so the database can notify you of cheaters in your game.")
export default class IdCommand extends ChannelCommand {
    @Argument({ type: new StringType(), description: "A link to your steam profile, or your SteamID."})
        steamUser!: string;

    async execute(message: Message, client: Client) {
        const steamID64 = await getSteamID64(this.steamUser).catch();
        if(steamID64 === undefined) {
            throw new Error("That is not a valid profile. Provide a SteamID or a link to a steam profile.");
        } else if (steamID64 === null) {
            throw new Error("This custom URL does not point to a valid profile.");
        }
        message.reply("SteamID64: `" + steamID64 + "`\n"
            + "SteamID3: `" + convertSteamID64toSteamID3(steamID64) + "`\n"
            + "SteamID1: `" + convertSteamID64toSteamID1(steamID64) + "`");
    }
}